...
def plot_bsa(
    ...
    if above_threshold:
        ...
        threshold_path = f"{out_prefix}_threshold.tsv"
    ...
    ax_bulk2.set_xticklabels(chr_list, rotation=45)
    ...
    ax_gprime.set_xticklabels(chr_list, rotation=45)
    ...
    snp_out = f"{out_prefix}_bulk_snpindex.{output_format}"
    stats_out = f"{out_prefix}_delta_ed_gprime.{output_format}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    optional_group.add_argument(
        ...
        type=str,
        default=None,
        help="Output prefix. Defaults to <input>.<bulk1>_vs_<bulk2>.",
    )
    optional_group.add_argument(
        "-t",
        "--thread",
...
def main() -> None:
    ...
    if args.prefix is None:
        args.prefix = f"{strip_known_suffix(args.file)}.{args.bulk1}_vs_{args.bulk2}"
    out_prefix = f"{args.out}/{args.prefix}".replace("\\", "/").replace("//", "/")

    log_path = f"{out_prefix}.postbsa.log"
    ...
    if rust_result is not None:
        ...
        logger.info(f"Raw table saved: {out_prefix}_raw.tsv")
        logger.info(f"Smoothed table saved: {out_prefix}_smooth.tsv")
    else:
        ...
        save_table(raw_df, f"{out_prefix}_raw.tsv", logger, "Raw table")
        save_table(smooth_df, f"{out_prefix}_smooth.tsv", logger, "Smoothed table")
