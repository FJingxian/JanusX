...
def _admx_debug_enabled() -> bool:
    raw = str(os.environ.get("JX_RSVD_RSS_DEBUG", os.environ.get("JX_PCA_RSS_DEBUG", ""))).strip()
    return raw != "" and raw.lower() not in {"0", "false", "off", "no"}


def _admx_current_memory_debug() -> tuple[str, str, str]:
    rss = "NA"
    footprint = "NA"
    metric = "rss"
    try:
        ...
        rss = _format_debug_bytes(rss_bytes)
    except Exception:
        rss_bytes = 0
    ...
    if sys.platform == "darwin":
        try:
            ...
            info = RUsageInfoV4()
            if int(fn(int(os.getpid()), 4, ctypes.byref(info))) == 0:
                metric = "phys_footprint"
                footprint = _format_debug_bytes(int(info.ri_phys_footprint))
        except Exception:
            pass
    return metric, rss, footprint


def _format_debug_bytes(bytes_value: int) -> str:
    b = float(max(0, int(bytes_value)))
    if b >= 1024.0**3:
        return f"{b / (1024.0**3):.2f} GiB"
    if b >= 1024.0**2:
        return f"{b / (1024.0**2):.1f} MiB"
    if b >= 1024.0:
        return f"{b / 1024.0:.1f} KiB"
    return f"{int(b)} B"


def _admx_py_mem_debug(stage: str, detail: str = "") -> None:
    if not _admx_debug_enabled():
        return
    metric, rss, footprint = _admx_current_memory_debug()
    msg = f"[ADMX-PYDEBUG] {stage} metric={metric} rss={rss} footprint={footprint}"
    if detail:
        msg += f" {detail}"
...
def _plink_prefix_path(genotype_path: str) -> Optional[Path]:
    ...
    if p.suffix.lower() in {".bed", ".bim", ".fam"}:
        p = p.with_suffix("")
    if Path(f"{p}.bed").exists() and Path(f"{p}.bim").exists() and Path(f"{p}.fam").exists():
        return p
    return None
...
def _bed_training_meta(
    ...
    *,
    ...
    prefix = _plink_prefix_path(genotype_path)
    if prefix is None:
        raise ValueError(f"BED prefix not found for genotype input: {genotype_path}")
    if bed_backend is not None and hasattr(bed_backend, "row_freq"):
        maf_arr = bed_backend.row_freq()
        n_samples = int(getattr(bed_backend, "n_samples"))
    elif hasattr(jxrs, "admx_bed_training_meta"):
        maf_arr, n_samples = jxrs.admx_bed_training_meta(
            str(prefix),
            bool(snps_only),
            float(maf),
            float(missing_rate),
        ...
        _row_idx, _miss, maf_arr, _row_flip, _site_keep, n_samples, _n_total = (
            jxrs.prepare_bed_logic_meta_selected(
                str(prefix),
                None,
                float(maf),
                float(missing_rate),
    ...
    f = np.ascontiguousarray(np.asarray(maf_arr, dtype=np.float32))
    if f.ndim != 1 or int(f.size) <= 0 or int(n_samples) <= 0:
        raise ValueError(
            f"Invalid BED training metadata: row_freq={f.shape}, n_samples={int(n_samples)}"
        )
    return f, int(n_samples)


def _open_bed_backend(
    genotype_path: str,
    *,
    snps_only: bool,
    maf: float,
    ...
    prefix = _plink_prefix_path(genotype_path)
    if prefix is None or not hasattr(jxrs, "AdmxBedBackend"):
        return None
    try:
        return jxrs.AdmxBedBackend(
            str(prefix),
            bool(snps_only),
            float(maf),
            float(missing_rate),
        ...
        if logger is not None:
            logger.warning(
                f"{'BED backend fallback':<22}: session open failed ({ex}); retry stateless path"
            )
        return None


def _open_bed_training_session(
    genotype_path: str,
    *,
    snps_only: bool,
    maf: float,
    ...
    _admx_py_mem_debug(
        "open_bed_training_session/start",
        f"path={genotype_path} snps_only={bool(snps_only)} maf={float(maf)} missing={float(missing_rate)}",
    )
    prefix = _plink_prefix_path(genotype_path)
    if prefix is None or not hasattr(jxrs, "AdmxBedTrainingSession"):
        _admx_py_mem_debug(
            "open_bed_training_session/unavailable",
            f"prefix={prefix} has_session={hasattr(jxrs, 'AdmxBedTrainingSession')}",
        )
        return None
    try:
        session_cls = jxrs.AdmxBedTrainingSession
        opener = getattr(session_cls, "open", None)
        if callable(opener):
            _admx_py_mem_debug("open_bed_training_session/before_rust_open", "method=open")
            session = opener(
                str(prefix),
                bool(snps_only),
                float(maf),
            ...
            _admx_py_mem_debug(
                ...
                f"n_snps={int(getattr(session, 'n_snps', -1))} n_samples={int(getattr(session, 'n_samples', -1))}",
            )
            return session
        _admx_py_mem_debug("open_bed_training_session/before_rust_open", "method=constructor")
        session = session_cls(
            str(prefix),
            bool(snps_only),
            float(maf),
        ...
        _admx_py_mem_debug("open_bed_training_session/exception", str(ex))
        if logger is not None:
            logger.warning(
                f"{'BED session fallback':<22}: training session open failed ({ex}); retry backend path"
            )
        return None
...
def _eig_s_right(x: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    d, v = np.linalg.eigh(x.T @ x)
    d = np.clip(d, 1e-12, None)
    s = np.sqrt(d)
    return (
...
def _admx_rsvd_k_prime(k_eff: int, max_cols: int) -> int:
    try:
        oversample = int(str(os.environ.get("JANUSX_ADMX_RSVD_OVERSAMPLE", "6")).strip())
    except Exception:
        oversample = 6
    try:
        kp_min = int(str(os.environ.get("JANUSX_ADMX_RSVD_KP_MIN", "12")).strip())
    except Exception:
        kp_min = 12
    try:
        kp_cap_raw = str(os.environ.get("JANUSX_ADMX_RSVD_KP_MAX", "")).strip()
        kp_cap = int(kp_cap_raw) if kp_cap_raw != "" else None
    except Exception:
        kp_cap = None

    limit = max(1, int(max_cols))
    k_base = max(1, int(k_eff))
    kp = max(k_base + max(0, int(oversample)), max(1, int(kp_min)))
    if kp_cap is not None:
        kp = min(kp, max(k_base, int(kp_cap)))
    return min(kp, limit)
...
def _rsvd(
    ...
    *,
    ...
    if g is not None:
        ...
        m, n = int(data_shape[0]), int(data_shape[1])
    else:
        raise ValueError("RSVD requires either dense genotype matrix or data_shape.")
    t0 = time.time()
    ...
    if use_stream_sample and genotype_path and hasattr(jxrs, "admx_rsvd_stream_sample"):
        ...
        if prefix is not None:
            try:
                ...
                if v.ndim != 2 or int(v.shape[0]) != int(n) or int(v.shape[1]) <= 0:
                    raise ValueError(
                        f"invalid sample-side RSVD output shape: eigvals={eigvals.shape}, eigvecs={v.shape}, expected ({n}, k)"
                    )
                varsum = float(np.sum(2.0 * np.asarray(f, dtype=np.float64) * (1.0 - np.asarray(f, dtype=np.float64))))
                if not (np.isfinite(varsum) and varsum > 0.0):
                    raise ValueError(f"invalid variance scale for RSVD stream backend: varsum={varsum}")
                if g is not None:
                    z = np.empty((m, int(v.shape[1])), dtype=np.float32)
                    jxrs.admx_multiply_a_omega_inplace(g, v, f, z)
                else:
                    if bed_backend is not None and hasattr(bed_backend, "multiply_a_omega"):
                        z = np.ascontiguousarray(
                            bed_backend.multiply_a_omega(v),
                            dtype=np.float32,
                        )
                    elif not hasattr(jxrs, "admx_multiply_a_omega_bed"):
                        raise ValueError(
                            "stream-sample BED RSVD requires admx_multiply_a_omega_bed backend"
                        )
                    else:
                        z = np.ascontiguousarray(
                            jxrs.admx_multiply_a_omega_bed(
                                str(genotype_path),
                                v,
                                f,
                                bool(snps_only),
                ...
                logger.info(f"{'RSVD backend':<22}: stream-sample BED")
    ...
    rng = np.random.default_rng(int(seed))
    k_prime = _admx_rsvd_k_prime(int(k), min(int(m), int(n)))
    alpha = 0.0
    ...
    s_all, v_small = _eig_s_right(g_small)
    v_small_k = np.ascontiguousarray(v_small[:, : int(k)], dtype=np.float32)
    z = np.ascontiguousarray(g_small @ v_small_k, dtype=np.float32)
    v = np.ascontiguousarray(q @ v_small_k, dtype=np.float32)
    elapsed = time.time() - t0
    logger.info(f"{'RSVD finished':<22}: {elapsed:.2f}s")
    _emit_progress(
        callback,
...
def _als_init(
    ...
    *,
    ...
    callback: ProgressCallback = None,
    data_shape: Optional[Tuple[int, int]] = None,
    ll_eval: Optional[Callable[[np.ndarray, np.ndarray], float]] = None,
) -> tuple[np.ndarray, np.ndarray]:
    t0 = time.time()
    if g is not None:
        m, n = g.shape
        ...
        raise ValueError("ALS init requires either dense genotype matrix or data_shape.")
    rng = np.random.default_rng(int(seed))
    ...
    for it in range(int(max_iter)):
        ...
        if high_corr:
            if rmse_err < rmse_best:
                rmse_best = rmse_err
                if p_best is None or q_best is None:
                ...
                stall_counter += 1
            if stall_counter >= 20 and p_best is not None and q_best is not None:
    ...
    if ll_eval is not None:
        ll0 = float(ll_eval(p, q))
        ...
        ll0 = float("nan")
        logger.info(f"{'Initial log-likelihood':<22}: skipped (BED stream path)")
    _emit_progress(
        callback,
        "als_done",
        elapsed=float(elapsed),
        initial_ll=(None if not np.isfinite(ll0) else float(ll0)),
        converged_iter=converged_iter,
...
def _adam_em_optimize(
    ...
    *,
    ...
    use_bed_backend = (
        (g is None)
        and (
            (bed_backend is not None and hasattr(bed_backend, "adam_optimize_f32"))
            or (
                genotype_path is not None
                and (_plink_prefix_path(str(genotype_path)) is not None)
                and hasattr(jxrs, "admx_adam_optimize_bed_f32")
            )
        )
    )
    if use_bed_backend:
        tile_label = "bed-stream"
        if bed_backend is not None and hasattr(bed_backend, "adam_optimize_inplace_f32"):
            tile_label = "bed-stream-inplace"
            ll_final, n_iter = bed_backend.adam_optimize_inplace_f32(
                p,
                q,
                float(lr),
                float(beta1),
            ...
            p_best = p
            q_best = q
        elif bed_backend is not None and hasattr(bed_backend, "adam_optimize_f32"):
            p_best, q_best, ll_final, n_iter = bed_backend.adam_optimize_f32(
                p,
                q,
                float(lr),
                float(beta1),
            ...
            p_best, q_best, ll_final, n_iter = jxrs.admx_adam_optimize_bed_f32(
                str(genotype_path),
                p,
                q,
                float(lr),
        ...
        if g is None:
            raise ValueError("Dense ADAM optimize requires genotype matrix.")
        tile_cols = int(g.shape[1]) if tile_cfg <= 0 else max(1, tile_cfg)
        tile_label = str(int(tile_cols))
    ...
    logger.info(
        ...
        f"ll={float(ll_final):>14.3f} tile={tile_label}"
    )
    elapsed = time.time() - t0
    logger.info(
    ...
    # Keep model matrices in float32 on the Python side. P is especially large for
    # million-SNP runs, and widening it to float64 creates a substantial avoidable RSS spike.
    return (
        np.ascontiguousarray(p_best, dtype=np.float32),
        np.ascontiguousarray(q_best, dtype=np.float32),
    )
...
def _fit_adamixture_on_u8(
    ...
    *,
    ...
    use_stream_sample_rsvd: bool = False,
    ...
    if warm_start_ok:
        ...
        z, v = _rsvd(
            ...
            power=int(cfg.power),
            tol=tol,
            genotype_path=str(cfg.genotype_path),
            snps_only=bool(cfg.snps_only),
            maf=float(cfg.maf),
            missing_rate=float(cfg.geno),
            use_stream_sample=bool(use_stream_sample_rsvd),
            logger=logger,
            callback=callback,
        )
...
def _fit_adamixture_on_bed_session(
    cfg: ADAMixtureConfig,
    logger: logging.Logger,
    callback: ProgressCallback = None,
    *,
    session: Any,
    data_label: str = "BED Rust session",
) -> tuple[np.ndarray, np.ndarray, int, int]:
    m = int(getattr(session, "n_snps"))
    n = int(getattr(session, "n_samples"))
    if m <= 0 or n <= 0:
        raise ValueError(f"Invalid BED training session shape: SNPs={m}, samples={n}")
    solver_mode = _resolve_solver_mode(cfg.solver)
    tol = float(max(1e-12, float(cfg.tol)))
    logger.info(f"{'Data loaded':<22}: SNPs={m}, samples={n} ({data_label})")
    logger.info(f"{'Training backend':<22}: rust-bed-session")
    _emit_progress(callback, "data_loaded", m=int(m), n=int(n), backend=str(data_label))
    if solver_mode == "adam-em":
        logger.info(f"{'Rust session':<22}: RSVD + ALS + Adam")
        _emit_progress(callback, "rsvd_start", power=int(cfg.power), backend="rust-bed-session")
    else:
        logger.info(f"{'Solver':<22}: ADAM (skip ALS)")
        _emit_progress(
            callback,
            "adam_start",
            max_iter=int(cfg.max_iter),
            check_every=int(max(1, int(cfg.check))),
            backend="rust-bed-session",
        )

    t0 = time.time()
    p_mat, q_mat, ll_final, n_iter, init_ll, als_iter = session.fit_k(
        int(cfg.k),
        int(cfg.seed),
        str(solver_mode),
        int(cfg.power),
    ...
    if p.shape != (int(m), int(cfg.k)):
        raise ValueError(f"Rust BED session returned invalid P shape: {p.shape}")
    if q.shape != (int(n), int(cfg.k)):
        raise ValueError(f"Rust BED session returned invalid Q shape: {q.shape}")
    if solver_mode == "adam-em":
        logger.info(f"{'ALS finished':<22}: iter={int(als_iter)} (rust session)")
        if np.isfinite(float(init_ll)):
            logger.info(f"{'Initial log-likelihood':<22}: {float(init_ll):.3f}")
        _emit_progress(
            callback,
            "rsvd_done",
            elapsed=None,
            converged_iter=None,
            max_power=int(cfg.power),
            backend="rust-bed-session",
        )
        _emit_progress(callback, "als_start", max_iter=int(cfg.max_als), backend="rust-bed-session")
        _emit_progress(
            callback,
            "als_done",
            elapsed=None,
            initial_ll=(None if not np.isfinite(float(init_ll)) else float(init_ll)),
            converged_iter=None,
            max_iter=int(cfg.max_als),
            backend="rust-bed-session",
        )
    ...
    logger.info(
        f"{'ADAM optimize':<22}: iter={int(n_iter):>4}/{int(cfg.max_iter):<4} "
        f"ll={float(ll_final):>14.3f} tile=bed-rust-session"
    )
    logger.info(
        f"{'Final log-likelihood':<22}: {float(ll_final):.3f} (elapsed {elapsed:.2f}s)"
    )
...
def _fit_adamixture_on_bed_backend(
    cfg: ADAMixtureConfig,
    logger: logging.Logger,
    callback: ProgressCallback = None,
    *,
    ...
    warm_start_ok = (init_p is not None) and (init_q is not None)
    if warm_start_ok:
        p0 = np.ascontiguousarray(np.asarray(init_p, dtype=np.float32))
        q0_raw = np.ascontiguousarray(np.asarray(init_q, dtype=np.float32))
        if p0.shape != (int(m), int(cfg.k)):
        ...
        z, v = _rsvd(
            ...
            power=int(cfg.power),
            tol=tol,
            data_shape=(int(m), int(n)),
            genotype_path=str(prefix),
            snps_only=bool(cfg.snps_only),
            maf=float(cfg.maf),
            missing_rate=float(cfg.geno),
            memory_mb=int(cfg.memory_mb),
        ...
        if solver_mode == "adam-em":
            ll_eval = None
            if bed_backend is not None and hasattr(bed_backend, "loglikelihood_f32"):
                ll_eval = lambda p_eval, q_eval: float(
                    bed_backend.loglikelihood_f32(
                        np.ascontiguousarray(p_eval, dtype=np.float32),
                        np.ascontiguousarray(q_eval, dtype=np.float32),
                    )
                )
            elif hasattr(jxrs, "admx_loglikelihood_bed_f32"):
                ll_eval = lambda p_eval, q_eval: float(
                    jxrs.admx_loglikelihood_bed_f32(
                        str(prefix),
                        np.ascontiguousarray(p_eval, dtype=np.float32),
                        np.ascontiguousarray(q_eval, dtype=np.float32),
                        bool(cfg.snps_only),
...
def _fit_adamixture_on_bed(
    ...
    *,
    ...
    _admx_py_mem_debug(
        "fit_bed/start",
        f"path={cfg.genotype_path} k={int(cfg.k)} warm_start={init_p is not None or init_q is not None}",
    )
    prefix = _plink_prefix_path(str(cfg.genotype_path))
    if prefix is None:
        raise ValueError(f"BED prefix not found for genotype input: {cfg.genotype_path}")
    if init_p is None and init_q is None:
        _admx_py_mem_debug("fit_bed/before_open_training_session", f"prefix={prefix}")
        session = _open_bed_training_session(
            str(prefix),
            snps_only=bool(cfg.snps_only),
            maf=float(cfg.maf),
            missing_rate=float(cfg.geno),
        ...
        if session is not None and hasattr(session, "fit_k"):
            try:
                _admx_py_mem_debug("fit_bed/before_fit_session", f"k={int(cfg.k)}")
                return _fit_adamixture_on_bed_session(
                    cfg,
                    logger,
                    callback=callback,
                    session=session,
                    data_label="BED Rust session",
                )
            except Exception as ex:
                if _is_admx_memory_limit_error(ex):
                    raise
                logger.warning(
                    f"{'BED session fallback':<22}: fit_k failed ({ex}); retry backend path"
                )
    bed_backend = _open_bed_backend(
        str(prefix),
        snps_only=bool(cfg.snps_only),
        maf=float(cfg.maf),
        missing_rate=float(cfg.geno),
    ...
    return _fit_adamixture_on_bed_backend(
        ...
        bed_backend=bed_backend,
        genotype_path=str(prefix),
        data_label="BED stream",
        init_p=init_p,
        init_q=init_q,
    )


def evaluate_adamixture_cverror_bed(
    cfg: ADAMixtureConfig,
    logger: logging.Logger,
    ...
    *,
    ...
    if bed_backend is None:
        raise ValueError("BED CV stream backend is unavailable for this input.")
    if not hasattr(bed_backend, "cv_observed_fold_counts") or not hasattr(
        bed_backend, "make_cv_fold_backend"
    ):
        raise ValueError("BED CV stream backend does not expose foldmask methods.")

    m = int(getattr(bed_backend, "n_snps"))
    n = int(getattr(bed_backend, "n_samples"))
    n_obs, fold_counts = bed_backend.cv_observed_fold_counts(int(folds), int(cfg.seed))
    fold_counts = np.ascontiguousarray(np.asarray(fold_counts, dtype=np.int64))
    n_obs = int(n_obs)
    if n_obs < int(folds):
        raise ValueError(
    ...
    if np.any(fold_counts <= 0):
        ...
        raise ValueError(
            "Failed to build non-empty CV folds in BED foldmask assignment; "
            f"empty folds={','.join(bad)}."
        )

    logger.info(
        f"{'CVerror':<22}: estimating from {int(folds)} folds "
        f"(observed={n_obs}, SNPs={m}, samples={n}, backend=bed-stream-foldmask)"
    )
    logger.info(
        f"{'CV fold sizes':<22}: "
    ...
    for i in range(int(folds)):
        fold_id = int(i)
        fold_backend = bed_backend.make_cv_fold_backend(
            int(fold_id),
            int(folds),
            int(cfg.seed),
        )
        holdout_n = int(getattr(fold_backend, "holdout_observed", 0))
        if holdout_n <= 0:
            raise ValueError(f"Fold {fold_id + 1} has no held-out genotypes.")

        cfg_fold = replace(cfg, seed=int(cfg.seed) + int(i + 1))
        p_fold, q_fold, _m, _n = _fit_adamixture_on_bed_backend(
            cfg_fold,
            logger,
            callback=None,
            bed_backend=fold_backend,
            genotype_path=str(prefix),
            data_label=f"BED CV fold {fold_id + 1}/{int(folds)}",
        )
        mean_nll, _deviance, n_fold_eval = fold_backend.holdout_nll_f32(
            np.ascontiguousarray(p_fold, dtype=np.float32),
            np.ascontiguousarray(q_fold, dtype=np.float32),
        )
        if int(n_fold_eval) <= 0:
            raise ValueError(f"Fold {fold_id + 1} produced no held-out evaluation genotypes.")
...
def evaluate_adamixture_cverror(
    ...
    use_bed_stream_cv = (
        (g_full is None)
        and (_plink_prefix_path(str(cfg.genotype_path)) is not None)
        and hasattr(jxrs, "AdmxBedBackend")
    )
    if use_bed_stream_cv:
        bed_backend = _open_bed_backend(
            str(cfg.genotype_path),
            snps_only=bool(cfg.snps_only),
            maf=float(cfg.maf),
            missing_rate=float(cfg.geno),
            memory_mb=int(cfg.memory_mb),
        ...
        if (
            bed_backend is not None
            and hasattr(bed_backend, "cv_observed_fold_counts")
            and hasattr(bed_backend, "make_cv_fold_backend")
        ):
            return evaluate_adamixture_cverror_bed(
                cfg,
                logger,
...
def train_adamixture(
    ...
    _admx_py_mem_debug(
        "train/start",
        f"path={cfg.genotype_path} k={int(cfg.k)} g_matrix={g_matrix is not None}",
    )
    _set_thread_env(cfg.threads)
    try:
        jxrs.admx_set_threads(int(max(1, int(cfg.threads))))
    ...
    use_bed_stream_train = (
        (g_matrix is None)
        and (_plink_prefix_path(str(cfg.genotype_path)) is not None)
        and (
            hasattr(jxrs, "AdmxBedTrainingSession")
            or
            hasattr(jxrs, "AdmxBedBackend")
            or (
                hasattr(jxrs, "admx_adam_optimize_bed_f32")
                and hasattr(jxrs, "admx_multiply_a_omega_bed")
            )
        )
    )
    _admx_py_mem_debug(
        "train/dispatch",
        f"use_bed_stream_train={bool(use_bed_stream_train)} has_session={hasattr(jxrs, 'AdmxBedTrainingSession')} has_backend={hasattr(jxrs, 'AdmxBedBackend')}",
    )
    if use_bed_stream_train:
        return _fit_adamixture_on_bed(
            cfg,
            logger,
            callback=callback,
            init_p=init_p,
    ...
    return _fit_adamixture_on_u8(
        ...
        init_p=init_p,
        init_q=init_q,
        use_stream_sample_rsvd=bool(g_matrix is None),
    )
