...
fn symmetric_eigh_f64_row_major_with_driver(
    ...
    #[cfg(any(target_os = "macos", target_os = "linux", target_os = "windows"))]
    {
        ...
        let try_dsyevr = || -> Result<(Vec<f64>, Option<Vec<f64>>, &'static str), String> {
            let mut a = a_row_major.to_vec();
        ...
        let try_dsyevd = || -> Result<(Vec<f64>, Option<Vec<f64>>, &'static str), String> {
            let mut a = a_row_major.to_vec();
            let mut w = vec![0.0_f64; n];
            let mut info: CblasInt = 0;
            let mut lwork: CblasInt = -1;
            ...
            if need_vec {
                let u_row = copy_col_major_to_row_major(&a, n);
        ...
        match driver {
            EighDriver::Dsyevr => {
                match try_dsyevr() {
                    ...
                    Err(err) => lapack_errors.push(format!("dsyevr: {err}")),
                }
                match try_dsyevd() {
                    Ok(res) => return Ok(res),
                    Err(err) => lapack_errors.push(format!("dsyevd: {err}")),
                }
            }
            EighDriver::Dsyevd => {
                match try_dsyevd() {
...
pub fn rust_eigh_from_array_f64<'py>(
    ...
    let n = nrows;
    let a_row_major: Vec<f64> = a_view.iter().copied().collect();
