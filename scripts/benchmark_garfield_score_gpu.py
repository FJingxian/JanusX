...
def build_packed_bits(
    ...
    if abs(float(density) - 0.5) < 1e-12:
        bits[:] = rng.integers(0, np.iinfo(np.uint64).max, size=(n_rows, row_words), dtype=np.uint64)
    else:
        chunk_rows = max(1, int(chunk_rows))
        for start in range(0, n_rows, chunk_rows):
            end = min(n_rows, start + chunk_rows)
            chunk = rng.random((end - start, row_words, 64), dtype=np.float32) < float(density)
            packed = np.packbits(chunk, axis=2, bitorder="little").reshape(end - start, row_words, 8)
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Benchmark Garfield batch score CPU vs Metal GPU."
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Metal threadgroup size; <=0 lets the optimized kernel auto-pick from threadExecutionWidth",
    )
    ...
    report = jx.garfield_compare_score_cont_centered_gain_batch_metal_vs_cpu(
        y,
        bits,
        int(args.samples),
        ...
        threads_per_threadgroup=(
            None if int(args.threads_per_threadgroup) <= 0 else int(args.threads_per_threadgroup)
        ),
    )
    report = dict(report)
